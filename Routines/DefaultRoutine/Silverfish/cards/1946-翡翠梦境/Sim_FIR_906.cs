...
namespace HREngine.Bots
{
	...
	class Sim_FIR_906 : SimTemplate
	{
		public override void onCardPlay(Playfield p, bool ownplay, Minion target, int choice, Handmanager.Handcard hc)
        {
			...
			if (ownplay)
			{
				foreach (Handmanager.Handcard handcard in p.owncards.ToArray())
				{
					if (handcard.card.type == CardDB.cardtype.SPELL && handcard.card.SpellSchool == CardDB.SpellSchool.NATURE)
					{
