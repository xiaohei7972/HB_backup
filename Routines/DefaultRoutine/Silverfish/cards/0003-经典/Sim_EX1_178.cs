    ...
    class Sim_EX1_178 : SimTemplate //* 战争古树 Ancient of War
    {
         ...
         public override void getBattlecryEffect(Playfield p, Minion own, Minion target, int choice)
        {
            if (p.ownFandralStaghelm > 0 && own.own)
            {
                ...
                {
                    PenalityManager.Instance.getChooseCard(own.handcard.card, choice).sim_card.onCardPlay(p, own.own, own, iChoice, hc);
                }
            }
            else PenalityManager.Instance.getChooseCard(own.handcard.card, choice).sim_card.onCardPlay(p, own.own, own, choice, hc);
                        } */
